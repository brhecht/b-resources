...
const DEFAULT_GROUPS = [
  { name: "Articles", color: "#5B9E8F", icon: "🔗", order: 0 },
  { name: "Videos", color: "#E8913A", icon: "🎬", order: 1 },
  { name: "Research", color: "#9B8EC4", icon: "🔬", order: 2 },
  { name: "Guides", color: "#6BA37E", icon: "📖", order: 3 },
  { name: "Tools", color: "#E57373", icon: "🛠️", order: 4 },
]
...
export default function References({ user }) {
  ...
  async function loadDocs() {
    ...
    try {
      ...
      console.error("References loadDocs error:", e)
      if (e?.code === "permission-denied") alert("Permission denied loading references. Check Firestore rules.")
    }
    setLoading(false)
  }
  ...
  async function loadGroups() {
    try {
      const q = query(collection(db, "groups"), where("collection", "==", "references"), orderBy("order", "asc"))
      const snap = await getDocs(q)
      ...
      try {
        const q2 = query(collection(db, "groups"), where("collection", "==", "references"))
  ...
  async function seedDefaultGroups() {
    ...
    for (const g of DEFAULT_GROUPS) {
      ...
      created.push({ id: docRef.id, ...g, parentId: null, collection: "references" })
    }
    return created
  }
  ...
  async function handleDeleteGroup(groupId) {
    ...
    try {
      ...
      for (const d of affected) {
        await updateDoc(doc(db, "references", d.id), { groupId: null, subGroupId: null, updatedAt: serverTimestamp() })
      }
  ...
  async function handleGroupChange(itemId, newGroupId, newSubGroupId) {
    try {
      ...
      await updateDoc(doc(db, "references", itemId), updates)
  ...
  async function handlePin(item) {
    ...
    try {
      await updateDoc(doc(db, "references", item.id), { pinned: newPinned, updatedAt: serverTimestamp() })
  ...
  async function handleTagsChange(itemId, newTags) {
    try {
      await updateDoc(doc(db, "references", itemId), { tags: newTags, updatedAt: serverTimestamp() })
  ...
  async function handleTitleChange(itemId, newTitle) {
    try {
      await updateDoc(doc(db, "references", itemId), { title: newTitle, updatedAt: serverTimestamp() })
  ...
  async function handleGroupChangeInModal(itemId, newGroupId, newSubGroupId) {
    try {
      await updateDoc(doc(db, "references", itemId), { groupId: newGroupId || "", subGroupId: newSubGroupId || "", updatedAt: serverTimestamp() })
  ...
  async function handleNotesChange(itemId, newDescription) {
    try {
      await updateDoc(doc(db, "references", itemId), { description: newDescription, updatedAt: serverTimestamp() })
  ...
  async function generateSummary(docId, title, description, fileName, fileType) {
    try {
      ...
      if (summary) {
        await updateDoc(doc(db, "references", docId), { summary })
  ...
  async function handleAdd(e) {
    ...
    try {
      ...
      if (file) {
        setUploading(true)
        storagePath = `references/${user.uid}/${Date.now()}_${file.name}`
      ...
      const docRef = await addDoc(collection(db, "references"), newDoc)
      ...
      console.error("References add error:", e)
  ...
  async function handleUpdate(e) {
    ...
    try {
      ...
      if (editFile) {
        ...
        const storagePath = `references/${user.uid}/${Date.now()}_${editFile.name}`
        const storageRef = ref(storage, storagePath)
      ...
      await updateDoc(doc(db, "references", editDoc.id), updates)
      setDocs(prev => prev.map(d => d.id === editDoc.id ? { ...d, ...updates } : d))
      setEditDoc(null)
      setEditFile(null)
      loadDocs().catch(() => {})
    } catch (e) {
      console.error("References update error:", e)
    ...
    <div style={{ minHeight: "100vh", background: BG, fontFamily: "system-ui, sans-serif", color: TEXT }}>
      <div style={{ background: CARD_BG, borderBottom: `1px solid ${BORDER}`, padding: "0 32px" }}>
        <div style={{ maxWidth: 1200, margin: "0 auto", display: "flex", alignItems: "center", justifyContent: "space-between", height: 64 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            ...
            <span style={{ fontSize: 20, fontWeight: 700, color: TEXT }}>References</span>
          </div>
      ...
      <div style={{ maxWidth: panelDoc ? "none" : 1200, margin: "0 auto", padding: "32px 32px" }}>
        ...
        {loading ? (
          ...
          <div style={{ textAlign: "center", color: MUTED, padding: 60 }}>No references found.</div>
        ) : view === "group" ? (
      ...
      {/* ADD MODAL */}
      {showAdd && (
        <Modal onClose={() => { setShowAdd(false); setFile(null); setProgress(0); setAddToGroupId(null) }} title="Add Reference" accent={ACCENT}>
          <form onSubmit={handleAdd} style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            ...
            <button type="submit" disabled={saving} style={{ background: ACCENT, color: "#fff", border: "none", borderRadius: 8, padding: "10px", cursor: saving ? "not-allowed" : "pointer", fontWeight: 600, opacity: saving ? 0.7 : 1 }}>
              {saving ? (uploading ? `Uploading ${progress}%...` : "Saving...") : "Save Reference"}
            </button>
          </form>
        </Modal>
      )}
      ...
      {viewDoc && (
        <Modal onClose={() => setViewDoc(null)} title="" accent={ACCENT} wide>
          <div>
            ...
            <CollapsibleMessages collectionName="references" docId={viewDoc.id} user={user} resourceTitle={viewDoc.title} accentColor={ACCENT} />
      ...
      {/* EDIT MODAL */}
      {editDoc && (
        <Modal onClose={() => { setEditDoc(null); setEditFile(null) }} title="Edit Reference" accent={ACCENT}>
